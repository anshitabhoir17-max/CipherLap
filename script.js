...
function getRiskMeta(score) {
  ...
  if (score < 55) {
    return { label: "Guarded", className: "risk-medium" };
  }
        ...
        .map(
          (block) => `
            <section class="result-block">
              <p class="result-title">${escapeHtml(block.title)}</p>
...
function renderResult(target, title, score, summary, findings, blocks = []) {
  const risk = getRiskMeta(score);
  target.innerHTML = `
    <h5>${escapeHtml(title)}</h5>
    <span class="risk-chip ${risk.className}">${escapeHtml(risk.label)} risk / ${score}</span>
...
function renderVerdict(target, title, verdict, toneClass, summary, findings = [], blocks = [], action = "") {
  target.innerHTML = `
    <h5>${escapeHtml(title)}</h5>
    <span class="risk-chip ${toneClass}">${escapeHtml(verdict)}</span>
    <p>${escapeHtml(summary)}</p>
    ${action ? `<p class="action-note"><strong>Action:</strong> ${escapeHtml(action)}</p>` : ""}
    ${renderList(findings)}
    ${blocks.length ? renderBlocks(blocks) : ""}
  `;
}
...
function analyzePhishingMessage(text) {
  if (!text.trim()) {
  ...
  for (const link of links.slice(0, 8)) {
    try {
      ...
      if (result.score >= 40) {
        ...
        suspiciousLinks.push(`${result.hostname} looks risky (${result.score}/100).`);
      }
    } catch {
  ...
  const verdict = suspicious ? "Suspicious Email" : "Likely Safe";
  const toneClass = suspicious ? "risk-high" : "risk-low";
  const action = suspicious
    ...
    blocks: [
      {
        ...
        content: `
          <p class="mono">From: ${escapeHtml(headers.from || "not supplied")}</p>
          <p class="mono">Reply-To: ${escapeHtml(headers["reply-to"] || "not supplied")}</p>
          <p class="mono">Links found: ${links.length}</p>
          <p class="mono">Suspicious links: ${suspiciousLinkCount}</p>
        `,
      },
    ],
  };
...
function analyzeEmailExposure(email) {
    ...
    blocks: [
      {
        ...
        content: `
          <p class="mono">Mailbox: ${escapeHtml(localPart)}</p>
          <p class="mono">Domain: ${escapeHtml(domain)}</p>
          <p class="mono">Provider class: ${emailProviderList.has(domain) ? "public provider" : "custom domain"}</p>
        `,
      },
      {
        title: "Recommended next step",
        content:
          "<p>Pair this tool with a backend endpoint that calls Have I Been Pwned or another breach service server-side, then display only the response summary in the browser.</p>",
      },
    ],
  };
}
...
function previewImage(container, src, altText, revokeOnLoad = false) {
  container.innerHTML = "";
  const image = document.createElement("img");
  image.src = src;
  image.alt = altText;

  if (revokeOnLoad && src.startsWith("blob:")) {
...
function analyzeImagePixels(imageData, width, height) {
  ...
  for (let y = yStart; y < yEnd; y += sampleStep) {
    for (let x = leftStart; x < midPoint; x += sampleStep) {
      ...
      const rightLum =
        0.2126 * data[rightIndex] + 0.7152 * data[rightIndex + 1] + 0.0722 * data[rightIndex + 2];
  ...
  if (!findings.length) {
    findings.push("No strong automated red flags were found in this quick visual pass.");
  }
...
function pickTopNotes(notes, limit = 3) {
  const unique = [];
...
function classifyImageAnalysis(file, dimensions, fullMetrics, centerMetrics, sideMetrics) {
  ...
  if ((hardCompositeSignal && editScore >= 30) || (strongCompositeSignal && editScore >= aiScore - 18)) {
    return {
      verdict: "Likely Edited / Composite",
      ...
      notes: pickTopNotes([...editNotes, ...aiNotes], 4),
    };
  }
  ...
  if (aiScore >= 58) {
    return {
      ...
      notes: pickTopNotes([...aiNotes, ...centerMetrics.findings], 4),
    };
  }

  return {
    verdict: "Likely Natural Photo",
    toneClass: "risk-low",
...
function combineImageDetections(apiDetection, localClassification) {
  ...
  if (localClassification.verdict === "Likely Edited / Composite" && apiDetection.score < 0.7) {
    return {
      ...localClassification,
      source: "Sightengine API + local layout analysis",
      notes: pickTopNotes(
        [
          ...localClassification.notes,
...
async function analyzeImageFile(file, previewTarget, resultTarget) {
  const { img, objectUrl } = await loadImageFile(file);

  try {
    previewImage(previewTarget, objectUrl, file.name);
    ...
    const centerMetrics = analyzeRegion(
      context,
      centerRect.x,
      centerRect.y,
      centerRect.width,
    ...
    const classification = classifyImageAnalysis(
      file,
    ...
    const finalDetection = combineImageDetections(apiDetection, classification);

    renderVerdict(
      resultTarget,
      "AI Image Detector",
      finalDetection.verdict,
      finalDetection.toneClass,
      finalDetection.summary,
      finalDetection.notes,
      [
        {
          title: "Image details",
          content: `
            <p class="mono">Name: ${escapeHtml(file.name)}</p>
            <p class="mono">Resolution: ${img.width} x ${img.height}</p>
            <p class="mono">Type: ${escapeHtml(file.type || "unknown")}</p>
          `,
        },
        {
          title: "Detection source",
          content: `<p class="mono">${escapeHtml(finalDetection.source)}</p>`,
        },
      ],
      finalDetection.action,
    );
  } finally {
    URL.revokeObjectURL(objectUrl);
  }
...
async function decodeStegoFile(file, resultTarget) {
  const { img, objectUrl } = await loadImageFile(file);
  ...
  try {
    ...
    if (best.cleaned && best.score >= 0.82) {
      renderResult(
        ...
        36,
        "A readable payload was found using a simple LSB pass. Treat it as a candidate extraction until you validate the context.",
        [note, `Best extraction strategy: ${best.strategy}.`, "Review the output carefully for deliberate padding or decoy text."],
        [
          {
            title: "Decoded text",
            content: `<p class="mono">${escapeHtml(best.cleaned)}</p>`,
          },
        ],
      );
      return;
    ...
    resultTarget.innerHTML = `
      <h5>Hidden Message Decoder</h5>
      <p>No obvious printable hidden text was recovered from a basic browser-side LSB pass.</p>
      <ul>
        <li>${escapeHtml(note)}</li>
        <li>Try PNG files, known stego samples, or payloads that were embedded with plain ASCII.</li>
        <li>Advanced schemes may use encryption, shuffled channels, metadata, or transform-domain methods.</li>
      </ul>
    `;
  } finally {
    URL.revokeObjectURL(objectUrl);
...
function attachEvents() {
  ...
  const stegoResult = $("#stegoResult");
  const morseResult = $("#morseResult");

  $("#phishingAnalyze").addEventListener("click", () => {
    try {
      const result = analyzePhishingMessage($("#phishingInput").value);
      renderVerdict(
        phishingResult,
      ...
      phishingResult.innerHTML = `<p>${escapeHtml(error.message)}</p>`;
    }
  });

  $("#breachAnalyze").addEventListener("click", () => {
    try {
      const result = analyzeEmailExposure($("#breachEmail").value);
      renderResult(
        breachResult,
        "Pwned Email Prep",
        result.score,
      ...
      breachResult.innerHTML = `<p>${escapeHtml(error.message)}</p>`;
    }
  });

  $("#urlAnalyze").addEventListener("click", () => {
    try {
      const result = inspectUrl($("#urlInput").value);
      renderResult(
        urlResult,
        "URL Risk Scanner",
        result.score,
        result.score >= 55
          ? "This link carries multiple strong phishing or trust-evasion signals."
          : "This link is not automatically malicious, but the scanner found areas worth checking.",
        result.flags,
        [
          {
            title: "Parsed URL",
            content: `
              <p class="mono">Normalized: ${escapeHtml(result.normalized)}</p>
              <p class="mono">Host: ${escapeHtml(result.hostname)}</p>
              <p class="mono">Protocol: ${escapeHtml(result.protocol)}</p>
            `,
          },
        ],
      );
    } catch (error) {
      urlResult.innerHTML = `<p>${escapeHtml(error.message)}</p>`;
    }
  });
  ...
  $("#imageAnalyze").addEventListener("click", async () => {
    ...
    try {
      await analyzeImageFile(file, imagePreview, imageResult);
    } catch (error) {
      imageResult.innerHTML = `<p>${escapeHtml(error.message)}</p>`;
    }
  });
  ...
  $("#imageInput").addEventListener("change", () => {
    const file = $("#imageInput").files[0];
    if (!file) {
      imagePreview.innerHTML = "<p>Preview will appear after upload.</p>";
      return;
    }

    const temporaryUrl = URL.createObjectURL(file);
    previewImage(imagePreview, temporaryUrl, file.name, true);
  });

  $("#stegoDecode").addEventListener("click", async () => {
    const file = $("#stegoInput").files[0];
    try {
      if (!file) {
        throw new Error("Choose an image before decoding.");
      }
      await decodeStegoFile(file, stegoResult);
    } catch (error) {
      stegoResult.innerHTML = `<p>${escapeHtml(error.message)}</p>`;
    }
  });
  ...
  $("#morseEncode").addEventListener("click", () => {
    ...
    morseResult.innerHTML = `
      <h5>Morse Code Lab</h5>
      <p class="mono">${escapeHtml(encodeMorse(input))}</p>
    `;
  });
  ...
  $("#morseDecodeBtn").addEventListener("click", () => {
    ...
    morseResult.innerHTML = `
      <h5>Morse Code Lab</h5>
      <p class="mono">${escapeHtml(decodeMorse(input))}</p>
    `;
  });

  $("#morseClear").addEventListener("click", () => {
    $("#morseInput").value = "";
    morseResult.innerHTML = "<p>Morse translation will appear here.</p>";
  });
}
